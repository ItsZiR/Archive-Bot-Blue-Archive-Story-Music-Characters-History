...
class Story_Query(commands.Cog):
    ...
    async def story_list(self, interaction: discord.Interaction):
        ...
        view = self.StoryPageView(pages)
        await interaction.response.send_message(embed=view.make_embed(), view=view)

    @app_commands.command(name="story_list_main", description="主線劇情大全")
    async def story_list_main(self, interaction: discord.Interaction):
        self.cursor.execute(sql_commands.story_list_type_sql(self.table, "'Main'"))
        story_list = self.cursor.fetchall()
